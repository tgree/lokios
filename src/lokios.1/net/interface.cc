...
net::interface::interface(size_t tx_qlen, size_t rx_qlen, uint16_t tx_mtu,
    uint16_t rx_mtu):
        ...
        wapi_node(method_delegate(handle_wapi_request),METHOD_GET_MASK,"net%u",
                  id),
        tcp_node(&wapi_node,func_delegate(tcp::handle_wapi_request),
                 METHOD_GET_MASK,"tcp"),
        tx_qlen(tx_qlen),
        rx_qlen(rx_qlen),
        rx_posted_count(0),
        ...
        tx_mtu(tx_mtu),
        rx_mtu(rx_mtu),
        tcp_ephemeral_ports_mem(NUM_EPHEMERAL_PORTS*sizeof(uint16_t)),
...
{
    kassert(s->state == tcp::socket::TCP_DRAIN_NIC_OPS);
