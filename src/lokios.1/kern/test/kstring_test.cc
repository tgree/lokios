...
class tmock_test
{
    ...
    TMOCK_TEST(test_kstring_to_u64)
    {
        kernel::string ks;
        try {ks.to_u64(); tmock::abort("exception expected");}
        catch (kernel::not_a_number_exception) {}

        ks.printf("12345");
        tmock::assert_equiv(ks.to_u64(),12345ULL);

        ks.printf("6");
        tmock::assert_equiv(ks.to_u64(),123456ULL);
